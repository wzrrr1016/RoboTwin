...
def _generate_examples(paths) -> Iterator[Tuple[str, Any]]:
    ...
    for path in paths:
        ...
        with h5py.File(path, "r") as f:
            ...
            head = f["/head_camera_image"][1:].astype(np.uint8)
            left = f["/left_wrist_image"][1:].astype(np.uint8)
            right = f["/right_wrist_image"][1:].astype(np.uint8)
            low = f["/low_cam_image"][1:].astype(np.uint8)
...
class aloha_blocks_ranking_rgb(MultiThreadedDatasetBuilder):
    ...
    def _split_paths(self):
        train_files = glob.glob("/mnt/data/VLA_flowmatching/RoboTwin/data/blocks_ranking_rgb/processed_openvla/train/*.hdf5")
        val_files = glob.glob("/mnt/data/VLA_flowmatching/RoboTwin/data/blocks_ranking_rgb/processed_openvla/val/*.hdf5")
...
if __name__ == "__main__":
    builder = aloha_blocks_ranking_rgb()
